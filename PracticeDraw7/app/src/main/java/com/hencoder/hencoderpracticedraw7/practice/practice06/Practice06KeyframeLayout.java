    ...
    protected void onAttachedToWindow() {
        ...
        animateBt.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // 初始帧：progress 为 0
                Keyframe initKeyFrame = Keyframe.ofFloat(0, 0);

                // 时间进行到一半：progress 为 100
                Keyframe halfKeyFrame = Keyframe.ofFloat(0.5f,100);

                // 结束帧：progress 回落到 80
                Keyframe endKeyFrame = Keyframe.ofFloat(1, 80);

                // 使用 PropertyValuesHolder.ofKeyframe() 来把关键帧拼接成一个完整的属性动画方案
                PropertyValuesHolder keyFrameHolder = PropertyValuesHolder.ofKeyframe("progress", initKeyFrame, halfKeyFrame, endKeyFrame);

                // 使用 ObjectAnimator.ofPropertyValuesHolder() 来创建动画
                ObjectAnimator animator = ObjectAnimator.ofPropertyValuesHolder(view, keyFrameHolder).setDuration(2000);
